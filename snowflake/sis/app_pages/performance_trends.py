...
def show_summary(df):
    ...
    with col1:
        st.metric("Overall Average Score", f"{df['avg_score_per_pop'].mean():.1f}")
    with col2:
        st.metric("Best Score", f"{df['avg_score_per_pop'].max():.1f}")
    with col3:
        st.metric("Total Balloons", f"{int(df['total_pops'].sum())}")
...
if st.session_state.color_performance_data is not None:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Min Score": st.column_config.NumberColumn("Min Score", format="%.1f"),
